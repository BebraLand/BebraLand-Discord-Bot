...
class TransferUserSelect(ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        logger.info(
            f"User selected for transfer: {selected_user.id} in channel {self.channel.id} {selected_user.name}({selected_user.id})"
        )
        ...
        try:
            logger.info(
                f"Starting ownership transfer: channel {self.channel.id}, from {self.owner_id} to {selected_user.id}"
            )
            ...
            await storage.update_temp_voice_channel(
                self.channel.id, owner_id=selected_user.id
            )
            ...
            logger.info(
                f"Removed manage permissions from old owner {self.current_owner.id}"
            )
            ...
            if self.current_owner.display_name in self.channel.name:
                new_name = self.channel.name.replace(
                    self.current_owner.display_name, selected_user.display_name
                )
            ...
            if temp_vc and temp_vc.get("control_message_id"):
                try:
                    control_message = await self.channel.fetch_message(
                        temp_vc["control_message_id"]
                    )
