...
class PermitMentionableSelect(ui.Select):
    ...
    def __init__(self, channel: discord.VoiceChannel, owner_id: int):
        ...
        placeholder = (
            "Select users to permit"
            if not constants.TEMP_VOICE_PERMIT_ROLES_ENABLED
            else "Select users/roles to permit"
        )
        super().__init__(
            ...
            for item in self.values:
                if isinstance(item, (discord.Member, discord.User)):
                    ...
                    await self.channel.set_permissions(
                        item, connect=True, view_channel=True
                    )
                    ...
                    if temp_vc:
                        ...
                        if item.id not in permitted_users:
                            ...
                            await storage.update_temp_voice_channel(
                                self.channel.id, permitted_users=permitted_users
                            )
                    ...
                    if temp_vc:
                        ...
                        if item.id not in permitted_roles:
                            ...
                            await storage.update_temp_voice_channel(
                                self.channel.id, permitted_roles=permitted_roles
                            )

            items_text = ", ".join(permitted_items)
            await interaction.response.edit_message(
                content=f"{lang_constants.SUCCESS_EMOJI} Permitted {items_text} to join the channel!",
                embed=None,
                view=None,
