...
class TempVoiceSettingsView(ui.View):
    ...
    def __init__(self, channel_id: int, owner_id: int):
        ...
        name_btn = ui.Button(
            label=f"{lang_constants.PENCIL_EMOJI} Name", style=discord.ButtonStyle.secondary, row=0)
        name_btn.callback = self.name_button_callback
        self.add_item(name_btn)

        limit_btn = ui.Button(
            label=f"{lang_constants.PEOPLE_EMOJI} Limit", style=discord.ButtonStyle.secondary, row=0)
        ...
        if constants.TEMP_VOICE_BITRATE_SETTINGS_ENABLED:
            bitrate_btn = ui.Button(
                label=f"{lang_constants.MUSIC_EMOJI} Bitrate", style=discord.ButtonStyle.secondary, row=0)
        ...
        if constants.TEMP_VOICE_REGION_SETTINGS_ENABLED:
            region_btn = ui.Button(
                label=f"{lang_constants.GLOBE_EMOJI} Region", style=discord.ButtonStyle.secondary, row=1)
        ...
        if constants.TEMP_VOICE_NSFW_SETTINGS_ENABLED:
            nsfw_btn = ui.Button(
                label=f"{lang_constants.NSFW_EMOJI} NSFW", style=discord.ButtonStyle.danger, row=1)
    ...
    def update_nsfw_button_style(self, channel: discord.VoiceChannel):
        ...
        for item in self.children:
            if isinstance(item, ui.Button) and item.label == f"{lang_constants.NSFW_EMOJI} NSFW":
    ...
    async def name_button_callback(self, interaction: discord.Interaction):
        ...
        if interaction.user.id != self.owner_id:
            await interaction.response.send_message(f"{lang_constants.ERROR_EMOJI} Only the channel owner can change settings!", ephemeral=True)
            return
