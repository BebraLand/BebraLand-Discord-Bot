        ...
        if success:
                ...
                if old_owner and old_owner.display_name in channel.name:
                    try:
                        new_name = channel.name.replace(
                            old_owner.display_name, new_owner.display_name
                        )
                        await channel.edit(name=new_name)
                        logger.info(f"Updated channel name to: {new_name}")
                    except Exception as e:
                        logger.error(
                            f"{lang_constants.ERROR_EMOJI} Error updating channel name: {e}"
                        )
            ...
            if temp_vc and temp_vc.get("control_message_id"):
                try:
                    ...
                    logger.error(
                        f"{lang_constants.ERROR_EMOJI} Error updating control panel message: {e}"
                    )
            return new_owner.id
