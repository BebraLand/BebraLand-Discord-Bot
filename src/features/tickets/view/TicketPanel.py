...
class TicketPanel(discord.ui.View):
    ...
    async def select_callback(self, select, interaction):
        ...
        category_data = next(
            (cat for cat in ticket_categories if cat["name"] == category_name), None
        )

        # Check if this category has forms
        if (
            category_data
            and category_data.get("forms")
            and len(category_data["forms"]) > 0
        ):
            ...
            logger.info(
                f"Showing form modal for category '{category_name}' to user {interaction.user.id}"
            )
        else:
            ...
            logger.info(
                f"Ticket creation attempt by {interaction.user.id} for category '{category_name}': {'Success' if success else 'Failed'}"
            )
