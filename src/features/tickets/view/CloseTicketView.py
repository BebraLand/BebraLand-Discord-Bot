...
class CloseTicketView(discord.ui.View):
    ...
    @discord.ui.button(label="Close Ticket", style=discord.ButtonStyle.danger, emoji=lang_constants.LOCK_EMOJI, custom_id="close_ticket_btn")
    async def close_button(self, button: discord.ui.Button, interaction: discord.Interaction):
        ...
        if interaction.user.id != self.user.id and not is_support:
            await interaction.response.send_message(lang_constants.ERROR_EMOJI + " Only the ticket creator or support staff can close this ticket.", ephemeral=True)
            return
