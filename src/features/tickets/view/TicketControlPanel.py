...
class TicketControlPanel(discord.ui.View):
    ...
    @discord.ui.button(
        ...
        emoji=lang_constants.TRANSCRIPT_EMOJI,
    ...
    async def transcript_button(
        ...
        try:
            ...
            await interaction.followup.send(
                lang_constants.SUCCESS_EMOJI
                + " Transcript created and sent to log channel!",
                file=user_file,
                ephemeral=True,
            )
            logger.info(
            ...
            await interaction.followup.send(
                lang_constants.ERROR_EMOJI + " Failed to create transcript.",
                ephemeral=True,
            )
    ...
    @discord.ui.button(
        ...
        emoji=lang_constants.UNLOCK_EMOJI,
        custom_id="reopen_btn",
    )
    async def reopen_button(
        ...
        if not is_support:
            await interaction.response.send_message(
                lang_constants.ERROR_EMOJI + " Only support staff can reopen tickets.",
                ephemeral=True,
            )
            return
        ...
        try:
            ...
            if not await db.reopen_ticket(self.ticket_id):
                await interaction.followup.send(
                    lang_constants.ERROR_EMOJI + " Failed to reopen ticket.",
                    ephemeral=True,
                )
                return
            ...
            await interaction.followup.send(
                lang_constants.ERROR_EMOJI + " Failed to reopen ticket.", ephemeral=True
            )

    @discord.ui.button(
        label="Delete",
        style=discord.ButtonStyle.danger,
        emoji=lang_constants.TRASH_EMOJI,
    ...
    async def delete_button(
        ...
        if not is_support:
            await interaction.response.send_message(
                lang_constants.ERROR_EMOJI + " Only support staff can delete tickets.",
                ephemeral=True,
            )
            return

        lang = await get_language(interaction.user.id)
        delete_msg = _("tickets.delete_countdown", lang)
        ...
        await interaction.channel.delete(
            reason=f"Ticket #{self.ticket_id} deleted by {interaction.user.name}"
        )
